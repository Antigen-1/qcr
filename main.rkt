...
(module* parallel #f
  ...
  (define (runParallel in out name)
    ...
    (define o-public (PEM_read_bio_RSAPublicKey (let ((temp (make-temporary-file)))
                                                  (display-to-file (read-bytes (string->number (read-line in-in)) in-in) temp #:exists 'truncate/replace)
                                                  temp)))
    ...
    (define crypto-bytes (crypto-random-bytes 32))
    (define m-key (RSA_public_encrypt 32 crypto-bytes o-public 1))
    (displayln (bytes-length m-key) out)
    (display m-key out)
    (define o-key (let ((len (string->number (read-line in-in))))
                    (RSA_private_decrypt len (read-bytes len in-in) m-private 1)))
    (define-values (revised-in-in revised-in-out) (make-pipe))
    (define-values (out-in out-out) (make-pipe))
    (void (thread (lambda () (vigenere-decrypt in-in revised-in-out o-key))))
