...
(module* parallel #f
  ...
  (define (copy-into-port in out)
    (let ((bytes-string (port->bytes in)))
      (define bytes (bytes-append (md5-bytes (open-input-bytes bytes-string)) (bytes 10) bytes-string))
      (define len (string->bytes/utf-8 (~a (bytes-length bytes))))
      (write-bytes len out)
      (write-byte 10 out)
      (vigenere-encrypt (open-input-bytes bytes) out (e-generator))
      (flush-output out)))
