...
(module* parallel #f
  ...
  (define (copy-into-port in out)
    (let ((bytes-string (port->bytes in)))
      (define data (bytes-append (md5-bytes (open-input-bytes bytes-string)) (bytes 10) bytes-string))
  ...
  (define (copy-from-port in out)
    (let ((len (string->number (bytes->string/utf-8 (read-bytes-line in)))))
      ...
      (define md5 (read-bytes-line bytes-port))
  ...
  (define (handleIO in out name)
    (let loop ()
      (define syn (sync/enable-break in (read-line-evt) (eof-evt in)))
      (cond ((input-port? syn) (define port (open-output-bytes))
            ...
            ((string? syn) (copy-into-port
                            ...
                            out)
                           (flush-output out))
            ((eof-object? syn) (custodian-shutdown-all (current-custodian)) (exit)))
      (loop)))
